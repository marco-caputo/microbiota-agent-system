...
def brain_add_cleaved_protein():
    ...
    Simulation.model.envs['brain']['context'].add(cleaved_protein)
    Simulation.model.move(cleaved_protein, cleaved_protein.pt, cleaved_protein.context)

# Brain steps
def step():
    Simulation.model.envs['brain']['context'].synchronize(restore_agent)

    def gather_agents_to_remove():
        return [agent for agent in Simulation.model.envs['brain']['context'].agents() if
    ...
    for agent in remove_agents:
        if Simulation.model.envs['brain']['context'].agent(agent.uid) is not None:
    ...
    for oligomer in oligomer_to_remove:
        if Simulation.model.envs['brain']['context'].agent(oligomer.uid) is not None:
        ...
        if agent.toAggregate and agent.is_valid():
            ...
            for x in agent_nghs_cleaved:
                if x.alreadyAggregate and x.uid != agent.uid:
                    if cont < 3:
                        if Simulation.model.envs['brain']['context'].agent(x.uid) is not None:
...
def add_cytokine():
    ...
    Simulation.model.envs['brain']['context'].add(cytokine)
